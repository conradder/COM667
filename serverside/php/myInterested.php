...
$qry1 = "SELECT eventdets.eventID, eventdets.evDate, eventdets.evTime, fsq_venuedets.venueName, fsq_venuedets.venueAdd, fsq_venuedets.venueCity, fsq_venuedets.venueLat, fsq_venuedets.venueLng, eventdets.notes, CONCAT(userdets.forname,' ', userdets.surname) as 'name' FROM eventdets inner join userdets on eventdets.createdBy = userdets.userID inner JOIN fsq_venuedets on eventdets.evLoc = fsq_venuedets.fsq_id inner join userreminder on userreminder.eventID = eventdets.eventID WHERE evDate >= CURRENT_DATE and userreminder.userID = $uID order by evDate asc";
